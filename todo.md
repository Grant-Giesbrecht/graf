...
**Current state:** 361 tests passing (was 209). All six release blockers fixed
...
- [ ] **`docs/source/modules.rst` is an empty stub** — a `toctree` with no
      ...
      wired up.
- [ ] **Write the format specification.** Highest-value doc: GrAF's entire
      selling point is language-agnostic readability, and there is currently no
      written definition of the format an independent implementation must meet.
